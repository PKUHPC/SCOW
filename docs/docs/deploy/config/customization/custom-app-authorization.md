...
- **创建新租户**时，会将所有交互式应用默认授权给新创建的租户。

- **创建新账户**或通过导入用户功能**导入新账户**时，会将已授权给账户所属租户的应用默认授权给新创建的账户。

- 在**为租户授权交互式应用**时，会同步将该应用的授权添加给租户下的账户。

![对租户授权交互式应用](images/app-authorization/authrize-tenant.png)

- 在**为租户取消授权交互式应用**时，也会同步取消该应用在租户下账户的授权。

![对租户取消授权交互式应用](images/app-authorization/revoke-tenant.png)

- 在操作日志中，追加授权应用、取消授权应用的操作行为。
