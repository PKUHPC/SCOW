...
| `changeEmail`         | boolean | 否       | 是否可以修改邮箱                  |
| `lockUser`            | boolean | 否       | 是否可以锁定用户登录               |
| `updatePasswordResetFlag`| boolean | 否       | 是否可以更改强制用户修改密码状态 |
...
如果认证系统声明支持`lockUser` capability，则需要实现这部分的API。

此capability是在实现认证失败锁定登录后，能够进行登录解锁和获取被锁定的用户。

### PATCH /lockUser/unlock
...
此API用于用户登录被锁定后，手动解封用户。

### GET /lockUser/getLockedUsers
...
此API用于获取所有登录被封锁的用户信息。

## 强制修改密码相关API

认证系统如果声明支持强制用户修改密码，则必须实现此部分的API。

### PATCH /updatePasswordResetFlag
...
| ------------- | ---- | ------ | -------- | ------ |
| `identityId`  | body | 字符串 | 是       | 用户ID |
| `forceFlag` | body | 字符串 | 是       | 强制修改密码标识，如果为true则设置为需要用户强制修改密码 |
...
| `204`  | 无   | 修改完成           |
| `404`  | 无   | 用户未找到         |
| `501`  | 无   | 不支持修改强制修改密码标识功能 |

#### 解释

此API用于完成修改强制修改密码标识的功能。