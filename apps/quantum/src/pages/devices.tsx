...
export const DevicesPage: NextPage = () => {
  ...
  const { data, isLoading, isError, error } = trpc.backend.device.findDevice.useQuery({ accountName: "_" });
  ...
  const orderedDeviceCardsData = useMemo(() => {
    ...
    const otherItems = DeviceCardsData.filter((item) => item.id !== "simulator:tc" && !item.id.includes("t60")); // 屏蔽已下线的芯片
      ...
      <Card style={{ marginBottom: 24 }}>
        ...
        <Row gutter={[16, 16]}>
          {isLoading ? (
            ...
            orderedDeviceCardsData.map((item, index) => (
              <Col lg={6} key={index}>
                <DeviceCard
                  id={item.id}
                  path={item.path}
