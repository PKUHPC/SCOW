...
export const CZGateFidelityMap: React.FC<Props> =
  ({ cz, coords, maxX, maxY, isFullWidth, nodeSize, fontSize, legendBelowChart }) => {
    ...
    const originalMaxF = max(fids) ?? 1.000;
    ...
    // 六边形路径
    function orientedRightAngleHexagonPath(x1: number, y1: number, x2: number,
      y2: number, length: number, width: number, tipWidth: number) {
      const cx = (x1 + x2) / 2;
      const cy = (y1 + y2) / 2;
