...
it("gets file list", async () => {
  ...
  expect(reply.results).toIncludeSameMembers([
    { name: "dir1", type: FileType.DIR, mode: expect.any(Number), mtime: expect.any(String), size: expect.any(Number) },
    {
      name: "test1",
      type: FileType.FILE,
...
it("returns error if list a file", async () => {
  await expectGrpcThrow(
    asyncUnaryCall(client, "readDirectory", {
      cluster,
      userId,
      path: actualPath("test1"),
    }),
    (e) => {
