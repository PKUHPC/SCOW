...
import {
  ...
  getScowdClientByUrl,
  mapConnectRpcStatusToGrpc,
} from "src/utils/scowd";
...
export const scowdDesktopServices = (cluster: string): DesktopOps => {
  let listContext:
    | {
        vncserverBinPath: string;
        desktopsDir: string;
        shadowDeskEnabled: boolean;
        shadowDeskConfig: ReturnType<typeof getDesktopConfig>["shadowDesk"];
      }
    | undefined;
  ...
  const getListContext = () => {
    ...
    const desktopConfig = getDesktopConfig(cluster);

    listContext = {
      vncserverBinPath,
      desktopsDir: desktopConfig.desktopsDir,
      shadowDeskEnabled: desktopConfig.shadowDesk?.enabled ?? false,
      shadowDeskConfig: desktopConfig.shadowDesk,
    };
  ...
  return {
    createDesktop: async (request) => {
      ...
      if (!scowdUrl) {
        throw {
          ...
          details: `Cluster ${cluster} not have login node ${host}`,
        } as ServiceError;
      }
      ...
      try {
        ...
        return {
          ...
          port: displayIdToPort(res.displayId),
        };
      } catch (err) {
    ...
    connectToDesktop: async (request) => {
      ...
      try {
        const res = await client.desktop.connectToDesktop({
          userId,
          vncPasswdPath: vncPasswdPath,
          displayId,
          id,
        });
    ...
    listUserDesktops: async (request) => {
      ...
      try {
        ...
        const shadowDeskPromise = shadowDeskEnabled
          ? getShadowDeskList(cluster, shadowDeskConfig)
          : Promise.resolve(undefined);

        const [vncRes, shadowRes] = await Promise.all([
          vncPromise,
          shadowDeskPromise,
        ]);
        ...
        const userDeskTops: Desktop[] = vncRes.userDesktops.map((desktop) => {
          const createTime = !desktop.createTime
            ...
            : new Date(
                Number(
                  desktop.createTime.seconds * BigInt(1000) +
        ...
        if (shadowDeskEnabled && shadowRes) {
          ...
          if (shadowRes.ok) {
            ...
            throw new Error(
              `HTTP error! status: ${shadowRes.status}, data: ${JSON.stringify(
                errorData
              )}`
            );
          }
