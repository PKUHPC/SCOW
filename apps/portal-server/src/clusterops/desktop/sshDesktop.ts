...
export const sshDesktopServices = (cluster: string): DesktopOps => ({
  createDesktop: async (request, logger) => {
    ...
    return await sshConnect(host, "root", logger, async (ssh) => {
      ...
      const originalListResp = await executeAsUser(ssh, userId, logger, true, vncserverBinPath, ["-list"]);
      ...
      const listResp = await executeAsUser(ssh, userId, logger, true, vncserverBinPath, ["-list"]);
