  ...
  return {
      ...
      const submitAndWriteMetadata = async (request: SubmitJobRequest) => {
        ...
        try {
          ...
          await client.file.writeFile({
            userId,
            filePath: join(workingDirectory, SESSION_METADATA_NAME),
          ...
          await client.file.writeFile({
            userId,
            filePath: join(lastSubmissionDirectory, APP_LAST_SUBMISSION_INFO),
    ...
    getAppLastSubmission: async (request, logger) => {
      ...
      try {
        ...
        if (!(await client.file.exists({ userId, path: file })).exists) {
    ...
    listAppSessions: async (request, logger) => {
      ...
      try {
        ...
        if (!(await client.file.exists({ userId, path: userAppJobDir })).exists) {
          return { sessions: [] };
        }
        ...
        if (runningJobIdsInEndedSessions.length > 0) {
          logger.trace(
            `Running jobs ${runningJobIdsInEndedSessions.join(",")} found in the ended_sessions file, ` +
              "possibly written due to delayed returns. They will be ignored in the display of completed jobs.",
          );
        }

        await Promise.all(
          list.map(async ({ name, fileType }) => {
            /**
             * 跳过的情况有：
            ...
            if (runningJobInfo && runningJobInfo.state === "RUNNING") {
              ...
              if (runningJobInfo && runningJobInfo.state === "PENDING") {
                logger.trace(
                  `Job ${sessionMetadata.jobId} is a pending job,` + " skipping in saving to ended_sessions.json",
                );
              } else {
                if (runningJobIdsInEndedSessions.includes(sessionMetadata.jobId)) {
                  logger.trace(
                    `Job ${sessionMetadata.jobId} has already been to written into ended_sessions file, ` +
        ...
        try {
          await writeEndedSessionsFileContent(
            client,
            userId,
            endedSessionsFilePath,
            existingEndedSessions,
            newEndedSessions,
            logger,
          );
        } catch (err) {
