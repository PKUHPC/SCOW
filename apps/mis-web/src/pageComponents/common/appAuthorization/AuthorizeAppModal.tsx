...
const AuthorizeAppModal: React.FC<Props> = ({
  ...
  const clusterName = getI18nConfigCurrentText(publicConfigClusters[clusterId]?.name, languageId) ?? clusterId;
        ...
        if (res.executed) {
          ...
          message.success(
            action === AuthorizeAction.AUTHORIZE
              ...
              : t(p("messages.unauthorizeSuccess")),
          );
          reload();
        } else {
          // 添加res.reason
    ...
    >
      <>
        ...
        {targetType === AppAuthTargetType.ACCOUNT && (
          <div style={{ marginBottom: "8px" }}>
            <span>
              {t(p("accountOwner"))}：{`${accountOwnerName}（ID: ${accountOwnerId}）`}
            </span>
          </div>
        )}
        <div style={{ marginBottom: "20px" }}>
          {t(p("cluster"))}：<span>{clusterName}</span>
        </div>
      </>
      ...
      >
        <Table.Column<AppAuthorizationInfo>
          ...
          render={(_, r) => {
            return (
              <>
                <Space style={{ width: "150px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                  {r.appName}
                </Space>
                    ...
                    modal.confirm({
                      ...
                      content: (
                        <>
                          <p>{contentTexts}</p>
                          {targetType === AppAuthTargetType.TENANT ? (
                            <p style={{ color: "red" }}>{t(p("confirm.unauthorize.tenantWarning"))}</p>
                          ) : undefined}
                        </>
                      ),
                      onOk: async () => {
