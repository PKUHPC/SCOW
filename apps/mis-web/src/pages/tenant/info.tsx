...
import { ChangeDefaultAccountBlockThresholdLink } from "src/pageComponents/tenant/ChangeDefaultAccountBlockThresholdModal";
...
export const TenantInfoPage: NextPage<Props> = requireAuth((u) => u.tenantRoles.includes(TenantRole.TENANT_ADMIN))((
  ...
  const { balance, accountCount, admins, userCount, tenantName, financialStaff, defaultAccountBlockThreshold } =
  ...
  return (
    <div>
      ...
      <Descriptions bordered column={1}>
        <Descriptions.Item label={t("common.tenantName")}>{tenantName}</Descriptions.Item>
        ...
        <Descriptions.Item label={t("common.accountCount")}>{accountCount}</Descriptions.Item>
        <Descriptions.Item label={t("common.userCount")}>{userCount}</Descriptions.Item>
  ...
  if (USE_MOCK) {
    return {
      props: {
        accountCount: 10,
        admins: [
          { userId: "123", userName: "1234" },
          { userId: "1234", userName: "12345" },
        ],
        financialStaff: [
          { userId: "123", userName: "1234" },
          { userId: "1234", userName: "12345" },
