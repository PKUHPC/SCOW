...
export const PlatformStatisticsPage: NextPage = requireAuth((u) =>
  ...
  const { data: totalCharge, isLoading: totalChargeAmountLoading } = useAsync({ promiseFn: getChargeTotalAmountFn });
  ...
  const { data: topSubmitJobUser, isLoading: topSubmitJobUserLoading } = useAsync({
    promiseFn: getUsersWithMostJobSubmissionsFn,
  });
  ...
  const topChargeAccountData = useMemo(() => {
    return (
      topChargeAccount?.results.map((r) => ({
        x: r.userName,
  ...
  const dailyChargeData = useMemo(() => {
    if (dailyCharge) {
      return formateData(
        dailyCharge?.results.map((d) => ({ date: d.date, count: moneyToNumber(d.amount) })),
        query.filterTime,
      );
    }
    return [];
  ...
  const dailyPayData = useMemo(() => {
    if (dailyPay) {
      return formateData(
        dailyPay?.results.map((d) => ({ date: d.date, count: moneyToNumber(d.amount) })),
        query.filterTime,
      );
    }
    return [];
  ...
  const topSubmitJobUserData = useMemo(() => {
    return (
      topSubmitJobUser?.results.map((r) => ({
        x: r.userName,
        y: r.count,
      })) || []
    );
    ...
    <>
      ...
      <Row gutter={[16, 16]}>
        <Col span={13} style={{ textAlign: "left", display: "flex", alignItems: "center" }}>
          <span>
            {t(p("statisticsDelay1"))}
            {formatLastTimeString ? t(p("statisticsDelay2"), [formatLastTimeString]) : ""}
          </span>
        </Col>
