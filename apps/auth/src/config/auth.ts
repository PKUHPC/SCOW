...
export const LdapConfigSchema = Type.Object(
  {
    ...
    `,
      ...
      `,
        ...
        `,
          ...
          addUserToLdapGroup: Type.Optional(
            Type.String({
              description: "LDAP增加用户时，应该把用户增加到哪个LDAP Group下。如果不填，创建用户后不会增加用户到Group",
            }),
          ),
      ...
      如果采用newGroupPerUser的用户组策略，创建的组的gid和uid和此相同。`,
          ...
          extraProps: Type.Optional(
            Type.Record(Type.String(), Type.Union([Type.Null(), Type.String(), Type.Array(Type.String())]), {
              description: `
    ...
    `,
        ...
        loginShell: Type.Optional(
          Type.String({
            description: "LDAP中对应用户在 Unix/Linux 系统上的默认shell。当前仅用于判断用户是否被禁止登录",
          }),
        ),
      },
      { description: "属性映射" },
    ),
    ppolicy: Type.Optional(
      Type.Object({
        defaultOlcPPolicyDn: Type.Optional(
          Type.String({ description: "密码策略条目DN，根据该 DN 加载对应的密码规则‌，默认所有用户或组使用此策略" }),
        ),
        pwdMaxFailures: Type.Optional(Type.Number({ description: "允许用户连续输入错误密码的最大次数。" })),
        pwdLockoutDurationMinutes: Type.Optional(
          Type.Number({ description: "用户被锁定后的持续时间（分钟），若设为 0 表示必须由管理员手动解锁‌。" }),
        ),
        pwdMustChangeAtFirstLoginOrResetByAdmin: Type.Optional(
          Type.Boolean({ description: "是否强制用户在首次登录或重置密码后修改密码。" }),
        ),
      }),
    ),
  },
...
export const SshConfigSchema = Type.Object(
  {
    baseNode: Type.Optional(
      Type.String({ description: "SSH认证中，以哪个节点为认证用户的基础。如果不设置则为第一个集群的第一个登录节点" }),
    ),
  },
...
export const OtpLdapSchema = Type.Object(
  {
    ...
    qrcodeDescription: Type.String({
      description: "secret二维码上方文字描述信息",
      default: "此二维码仅出现一次，用过即毁",
    }),
    label: Type.String({
      description: "otp验证软件扫描二维码之后，出现的label中，用户名和@后显示的名称",
      default: "SCOW",
    }),
...
export const UiConfigSchema = Type.Object({
  default: Type.Object({
    ...
    slogan: Type.Object(
      {
        color: Type.String({ description: "默认标语文字颜色", default: "#595959" }),
...
export const AuthConfigSchema = Type.Object({
  ...
  captcha: Type.Object(
    {
      enabled: Type.Boolean({ description: "验证码功能是否启用，默认为启用", default: true }),
    },
...
export const getAuthConfig = () => {
  ...
  if (config.authType === AuthType.ldap) {
    ...
    if (config.ldap.addUser) {
      if (
        config.ldap.addUser.groupStrategy === NewUserGroupStrategy.newGroupPerUser &&
      ...
      if (
        config.ldap.addUser.groupStrategy === NewUserGroupStrategy.oneGroupForAllUsers &&
