      ...
      await useLdap(
        ...
        if (
          !(await validateOtpCode(
            { userId: user.identityId, dn: user.dn },
            otpCode,
            callbackUrl,
            req,
            res,
        ...
        await useLdap(logger, ldapConfig, { dn: user.dn, password })(async () => {
          ...
          const pwdReset = await searchOne(
            logger,
            client,
            user.dn,
            {
          ...
          if (pwdReset?.[0] === "TRUE") {
            await serveLoginHtml({ err: false }, callbackUrl, req, res, undefined, undefined, undefined, {
              username,
              password,
          ...
          if (err.name === "InvalidCredentialsError") {
            const pwdAttributes = await searchOne(
              logger,
              client,
              user.dn,
              {
            ...
            const remainCount =
              Number(ldapConfig.ppolicy?.pwdMaxFailures || 0) - (pwdAttributes?.pwdFailuretime?.length || 0);
            ...
            if (pwdAttributes?.isLocked) {
              if (ldapConfig.ppolicy?.pwdLockoutDurationMinutes === 0) {
                await serveLoginHtml(
                  { err: true, errMessage: "accountLocked" },
                  callbackUrl,
                  req,
                  res,
                  undefined,
                ...
                await serveLoginHtml(
                  {
                    err: true,
                    errMessage: "accountLockedTime1",
                    dynamicsErrMessage: "accountLockedTime2",
                    lockedMinutes: ldapConfig.ppolicy?.pwdLockoutDurationMinutes,
                  },
                  callbackUrl,
                  req,
                  res,
              ...
              await serveLoginHtml(
                { err: true, errMessage: "invalidPasswordRemain" },
                callbackUrl,
                req,
                res,
                undefined,
                undefined,
                remainCount > 0 ? remainCount : 1,
              );
            } else {
              await serveLoginHtml(
                { err: true, errMessage: "invalidPassword" },
                callbackUrl,
                req,
                res,
                undefined,
