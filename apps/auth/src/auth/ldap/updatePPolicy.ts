...
export async function modifyPPolicy(log: FastifyBaseLogger, ldap: LdapConfigSchema): Promise<boolean> {
    ...
    if (ldap.ppolicy?.defaultOlcPPolicyDn) {
      try {
        if (typeof ldap.ppolicy?.pwdMaxFailures === "number" && ldap.ppolicy.pwdMaxFailures > 0) {
          await modify(
            ldap.ppolicy.defaultOlcPPolicyDn,
            new ldapjs.Change({
              operation: "replace",
              modification: {
        ...
        if (
          typeof ldap.ppolicy?.pwdLockoutDurationMinutes === "number" &&
