...
export const OverviewTable: React.FC<Props> = ({
  ...
  const noAuthorizedPartitionClusterIds = useMemo((): Set<string> => {
    return new Set(
      summaryClusterInfo.filter((cluster) => cluster.partitions.length === 0).map((cluster) => cluster.clusterId),
    );
  }, [summaryClusterInfo]);
  ...
  return isLoading || currentClusters.length > 0 ? (
    ...
    <DashboardSection title={t(p("platformOverview"))}>{t(p("contact"))}</DashboardSection>
  );
};
