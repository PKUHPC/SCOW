...
import {
  ...
  getPublishStatusUpperText,
  statusColors,
  transformPublishStatusText,
...
export const AlgorithmVersionList: React.FC<Props> = ({ algorithms, algorithmId, algorithmName, cluster }) => {
  ...
  const deleteAlgorithmVersion = useCallback(
    (id: number, isConfirmed?: boolean, isUsedPath?: boolean) => {
      confirm({
        title: isConfirmed ? (isUsedPath ? t(p("confirmedText2")) : t(p("confirmedText"))) : t(p("delete")),
        onOk: async () => {
        ...
        <Table
          ...
          columns={[
            ...
            { dataIndex: "privatePath", title: t(p("path")) },
            {
              ...
              title: t(p("sharedStatus")),
              render: (status: SharedStatus) => (
                <span style={{ color: statusColors[status] }}>{t(pCommon(transformPublishStatusText(status)))}</span>
              ),
            },
            {
            ...
            },
            {
              ...
              render: (_, r) => {
                const publishTitle =
                  r.sharedStatus === SharedStatus.SHARED ? t(p("cancelPublishTitle")) : t(p("publish"));
                return (
