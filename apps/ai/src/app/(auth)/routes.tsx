...
export const userRoutes: (
  ...
  t: TransType,
) => NavItemProps[] = (user, publicConfig, clusterConfigs, currentClusters, t) => {
  if (!user) {
    return [];
  }
  ...
  return [
      ...
      ? publicConfig.NAV_LINKS.map((link) => {
          ...
          return {
            ...
            children: link.children?.length
              ? link.children?.map(
                  (
                    childLink: Omit<NavLink, "children" | "url"> & {
                      url: string;
                    },
                  ) =>
                    ({
    ...
    ...(user.platformRoles?.includes(PlatformRole.PLATFORM_ADMIN)
      ? [
          {
            Icon: DataAssetIcon,
            text: t("routes.publicAsset.title"),
            path: "/publicAsset",
            clickToPath: "/publicAsset/dataset",
            children: [
              {
                Icon: DatasetIcon,
                text: t("routes.publicAsset.dataTitle"),
                path: "/publicAsset/dataset",
              },
              {
                Icon: ImageIcon,
                text: t("routes.image.title"),
                path: "/publicAsset/image",
              },
              {
                Icon: AlgorithmIcon,
                text: t("routes.algorithm.title"),
                path: "/publicAsset/algorithm",
              },
              {
                Icon: ModelIcon,
                text: t("routes.model.title"),
                path: "/publicAsset/model",
              },
            ],
          },
        ]
