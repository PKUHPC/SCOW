...
interface Props {
  createInferParams?: ResubmitInferParams;
  misPath: string;
}
...
export const LaunchInferForm = ({ createInferParams, misPath }: Props) => {
  ...
  // createInferParams is used directly by resubmit effects (no template merge)
  const [saveTemplateOpen, setSaveTemplateOpen] = useState(false);
  const [templateListOpen, setTemplateListOpen] = useState(false);
  const [templateSnapshot, setTemplateSnapshot] = useState<InferTemplateFormData | null>(null);
  ...
  const createInferJobMutation = trpc.jobs.submitInferJob.useMutation({
    onSuccess: () => {
      message.success(t(pInfer("submitInferSuccessfully")));
    ...
    onError: (error) => {
      ...
      message.error(`${t(pInfer("submitInferFailed"))}: ${error.message}`);
    },
  });
  ...
  useEffect(() => {
    ...
    if (!applyingTemplate) {
      ...
      resubmitServicePortAppliedRef.current = shouldReset;
    }
    if (shouldReset && !applyingTemplate) {
      ...
      appForm.setFieldsValue({
        image: undefined,
        command: undefined,
        containerServicePort: undefined,
  ...
  const handleImageSourceChange = (nextSource: InferImageSourceKey) => {
    ...
    const currentDraft: ImageSourceDraft = {
      ...
      containerServicePort: appForm.getFieldValue("containerServicePort"),
    };
  ...
  }, [appForm, createInferParams, modelSelectionLookup]);

  useEffect(() => {
    if (!createInferParams) {
      resubmitMountEnvAppliedRef.current = false;
      resubmitServicePortAppliedRef.current = false;
      return;
    }
    ...
    const envVariablesDraft = (createInferParams.envVariables ?? [])
    ...
    if (!resubmitServicePortAppliedRef.current) {
      const port = createInferParams.containerServicePort;
      if (port !== undefined && port !== null && !Number.isNaN(Number(port))) {
        appForm.setFieldValue("containerServicePort", Number(port));
      }
      resubmitServicePortAppliedRef.current = true;
    }
  ...
  const resubmitImagePreference = useMemo(() => {
    ...
    if (createInferParams.remoteImageUrl) {
      const draft: ImageSourceDraft = {
        image: createInferParams.remoteImageUrl,
        usePrivateImage: Boolean(createInferParams.privateImageRepositoryCredentials),
        remoteUsername: createInferParams.privateImageRepositoryCredentials?.userName,
        remotePassword: createInferParams.privateImageRepositoryCredentials?.password,
      };
      return {
    ...
    if (createInferParams.image !== undefined && createInferParams.image !== null) {
      const draft: ImageSourceDraft = {
  ...
  const currentCommandDefault = useMemo(() => {
    const trimmedResubmitCommand = createInferParams?.command?.trim();
    ...
    if (
      ...
      if (selectedImageSource === "remote") {
        ...
        if (currentRemote && createInferParams.remoteImageUrl === currentRemote) {
  ...
  const normalizedNodeCount = Math.max(1, Number(selectedNodeCount ?? 1));
  ...
  }, [accountClusterMap, appAvailableAccountsAndClusters, availableAccountOptions, createInferParams, resourceForm]);
  ...
  useEffect(() => {
    ...
    if (createInferParams.clusterId && selectedCluster && createInferParams.clusterId !== selectedCluster) {
      return;
    }
    ...
    if (createInferParams.qos) {
      if (qosList.includes(createInferParams.qos)) {
        updates.priority = createInferParams.qos;
      } else {
    ...
    if (maxTimeValue !== undefined) {
      ...
      updates.maxTimeUnlimited = false;
    } else {
      updates.maxTime = undefined;
      updates.maxTimeUnlimited = isSavedMaxTimeUnlimited;
    }

    updates.nodeCount = Math.max(1, createInferParams.nodeCount ?? 1);

    if (targetQueue.type === "gpu") {
      const gpuUnits = Number(createInferParams.gpuCount ?? 0);
      updates.gpuCores = gpuUnits > 0 ? Math.max(1, gpuUnits) : undefined;
      updates.cpuCores = undefined;
    } else {
      const cpuUnits = Number(createInferParams.coreCount ?? 0);
  ...
  useEffect(() => {
    if (resourceForm.getFieldValue("maxTimeUnlimited")) {
      return;
    }
  ...
  useEffect(() => {
    ...
    if (createInferParams && !resubmitQueueAppliedRef.current) {
      return;
    }
  ...
  }, [createInferParams, currentQueueOptions, getAvailablePartitionIsLoading, selectedQueueKey]);

  // 若用户尚未选择账户，自动填入第一条有效账户
  useEffect(() => {
    if (createInferParams && !resubmitResourceAppliedRef.current) {
      return;
    }
  ...
  }, [availableAccountOptions, createInferParams, resourceForm, selectedAccount]);
  ...
  useEffect(() => {
    ...
    if (createInferParams) {
      ...
      const savedPriority = createInferParams.qos;
      const savedQueueId = createInferParams.partition;
  ...
  }, [createInferParams, resourceForm, selectedQueueKey, selectedQueueOption]);

  const buildInferTemplateFormData = (): InferTemplateFormData => {
    ...
    return {
      ...
      containerServicePort: appValues.containerServicePort ?? 8080,
    };
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error("Failed to validate create infer job form:", error);
      return;
    }
    ...
    try {
      ...
      const parsedNodeCount = Number(nodeCountValue ?? 0);
      if (!Number.isFinite(parsedNodeCount) || parsedNodeCount <= 0) {
        message.error(t(p("invalidResourceConfig")));
        return;
      }
      const normalizedNodeCount = Math.max(1, Math.round(parsedNodeCount));
      ...
      if (!submitUnlimited) {
        const maxTimeValue = Number(maxTime);
      ...
      await createInferJobMutation.mutateAsync({
        clusterId: cluster,
        InferenceJobName: baseValues.appJobName,
        image: imageId,
        isImagePrivate,
        localImageName,
        remoteImageUrl,
  ...
  const handleTemplateUse = async (formData: TemplateFormData, templateCluster: string) => {
    ...
    const applyFn = async () => {
      ...
      const tplMaxTimeUnlimited = cleanedFormData.maxTimeUnlimited as boolean | undefined;
      if (tplMaxTimeUnit) {
      ...
      const tplNodeCount = cleanedFormData.nodeCount as number | undefined;
      ...
      if (tplPartition) {
        ...
        if (partitionsForCluster) {
          ...
          if (matched) {
            ...
            if (tplMaxTimeUnlimited != null) {
              resourceForm.setFieldsValue({ maxTimeUnlimited: tplMaxTimeUnlimited });
            }
            if (tplMaxTime != null && !tplMaxTimeUnlimited) {
              resourceForm.setFieldsValue({ maxTime: tplMaxTime });
            }
            if (tplNodeCount != null) {
              resourceForm.setFieldsValue({ nodeCount: tplNodeCount });
            }
      ...
      if (!clusterChanged) {
        const port = cleanedFormData.containerServicePort;
        if (port !== undefined && port !== null && !Number.isNaN(Number(port))) {
          appForm.setFieldValue("containerServicePort", Number(port));
        }
      }
      ...
      if (!clusterChanged && tplModels.length) {
        const fetchedModels = await trpcUtils.model.getAllModelVersions
          .fetch({ clusterId: finalCluster })
          .catch(() => undefined);
        if (fetchedModels) {
          const categories: ResourceCategory[] = [];
      ...
      let finalImageSource: InferImageSourceKey | undefined;
          ...
          <FooterStatValue
            ...
            onClick={async () => {
              try {
                ...
                setTemplateSnapshot(buildInferTemplateFormData());
