...
export const getAllModelVersions = procedure
  ...
  .query(async ({ input: { clusterId }, ctx: { user } }) => {
    ...
    const personalModels = await em.find(
      Model,
      {
        $and: [
    ...
    const publicModels = await em.find(
      Model,
      {
    ...
    const modelIds = [...personalModels.map((model) => model.id), ...publicModels.map((model) => model.id)];
...
export const createModelVersion = procedure
  ...
  .mutation(async ({ input, ctx: { user } }) => {
    ...
    if (!isPlatformOwned && model.owner !== user.identityId) {
      const detailMessage = `Model id:${input.modelId} is not owned by current user. currentUserId:${user.identityId}`;
    ...
    const modelVersionExist = await em.findOne(ModelVersion, { versionName: input.versionName, model });
...
export const updateModelVersion = procedure
  ...
  .mutation(async ({ input, ctx: { user } }) => {
    ...
    if (!isPlatformOwned && model.owner !== user.identityId) {
      const detailMessage = `Model id:${modelId} is not owned by current user. currentUserId:${user.identityId}`;
      logger.error(detailMessage);
      throw new TRPCError({
        code: "FORBIDDEN",
        message: "Operation failed: The model asset does not belong to the current user.",
    ...
    if (!modelVersion) throw new TRPCError({ code: "NOT_FOUND", message: `ModelVersion ${versionId} not found` });
    ...
    if (modelVersion.sharedStatus === SharedStatus.SHARING || modelVersion.sharedStatus === SharedStatus.UNSHARING) {
      throw new TRPCError({
    ...
    const needUpdateSharedPath =
      modelVersion.sharedStatus === SharedStatus.SHARED && versionName !== modelVersion.versionName;
    // 更新已分享目录下的版本路径名称
...
export const deleteModelVersion = procedure
  ...
  .mutation(async ({ input, ctx: { user } }) => {
    ...
    if (!modelVersion) throw new TRPCError({ code: "NOT_FOUND", message: `ModelVersion ${input.versionId} not found` });

    const model = await em.findOne(Model, { id: input.modelId }, { populate: ["versions", "versions.sharedStatus"] });
    if (!model) {
      throw new TRPCError({ code: "NOT_FOUND", message: `Model ${input.modelId} not found` });
    }
    ...
    if (modelVersion.sharedStatus === SharedStatus.SHARED) {
      ...
      if (!isPlatformOwned) {
        try {
          ...
          const pathToUnshare =
            model.versions.filter((v) => v.id !== input.versionId && v.sharedStatus === SharedStatus.SHARED).length > 0
      ...
      model.isShared = model.versions.filter((v) => v.sharedStatus === SharedStatus.SHARED).length > 1 ? true : false;
      em.persist(model);
    }
...
export const shareModelVersion = procedure
  ...
  .mutation(async ({ input: { modelId, versionId, isPlatformOwned }, ctx: { user } }) => {
    ...
    const modelVersion = await em.findOne(ModelVersion, { id: versionId });
    if (!modelVersion) throw new TRPCError({ code: "NOT_FOUND", message: `ModelVersion ${modelId} not found` });

    if (modelVersion.sharedStatus === SharedStatus.SHARED)
      throw new TRPCError({ code: "CONFLICT", message: "ModelVersion is already shared" });

    const model = await em.findOne(Model, { id: modelId });
    if (!model) throw new TRPCError({ code: "NOT_FOUND", message: `Model ${modelId} not found` });
    ...
    // 若公共资产路径改变，则无法发布
    const checkIsPublicAssetModelVersion = checkIsPublicPaths(model.clusterId, [modelVersion.path]);
...
export const unShareModelVersion = procedure
  ...
  .mutation(async ({ input: { versionId, modelId, isPlatformOwned }, ctx: { user } }) => {
    ...
    const successCallback = async () => {
      ...
      modelVersion.sharedStatus = SharedStatus.UNSHARED;
      modelVersion.path = modelVersion.privatePath;
      model.isShared = model.versions.filter((v) => v.sharedStatus === SharedStatus.SHARED).length > 0 ? true : false;
...
export const copyPublicModelVersion = procedure
  ...
  .mutation(async ({ input, ctx: { user } }) => {
    ...
    // 1. 检查模型版本是否为公开版本
    const modelVersion = await em.findOne(
      ModelVersion,
    ...
    // 若公共资产路径改变，则无法复制
    const checkIsPublicPathsResult = checkIsPublicPaths(modelVersion.model.$.clusterId, [modelVersion.path]);
