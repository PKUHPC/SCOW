...
export class ScowdImageDriver implements ImageDriver {
      ...
      async (abortController) => {
        ...
        try {
          if (source === Source.INTERNAL) {
            // LOAD_IMAGE START
            appendImageCreationOutput(imageId, CreationOperation.LOAD_IMAGE, LOAD_DEFAULT_OUTPUT, this.logger);
            // 本地镜像检查源文件拥有者权限
            ...
            const loadResponse = await wrap(
              ...
              if (abortController.signal.aborted) {
                this.logger.error(`Load operation for image (ID: ${imageId}, Name: ${name}:${tag}) was aborted`);
                cleanupImageCreationOutput(imageId, this.logger);
                throw new TRPCError({
                  code: "BAD_REQUEST",
              ...
              this.logger.error(
                `Load image (ID: ${imageId}, Name: ${name}:${tag}) ` + `failed while creating the image, ${e.message}`,
              );
              throw new TRPCError({
                code: "BAD_REQUEST",
                message:
                  `Load image (ID: ${imageId}, Name: ${name}:${tag}) failed ` +
                  `while creating the image, ${e.message}`,
              });
            });

            // LOAD_IMAGE COMPLETED
            appendImageCreationOutput(imageId, CreationOperation.LOAD_IMAGE, LOAD_DEFAULT_OUTPUT, this.logger, true);
  ...
  async saveImage({ node, rowContainerId, localImageUrl, harborImageUrl, imageId }: saveImageParams): Promise<void> {
    return withAbortHandling(
      imageId,
      async (abortController) => {
        try {
          ...
          await wrap(
            ...
            this.logger.error(
              `Commit container image (ID: ${imageId}, from ${localImageUrl} to ${harborImageUrl}) ` +
