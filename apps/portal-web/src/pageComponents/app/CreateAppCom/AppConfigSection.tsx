...
export const AppConfigSection = ({
  ...
  const customFormItems = useMemo(
    () =>
      attributes.map((item, index) => {
        ...
        const selectOptions = item.select.filter(
          (x) => !x.requireGpu || (x.requireGpu && currentPartitionInfo?.gpus)
        );
        ...
        const initialValue =
          item.type === "SELECT"
            ? getSelectAttributeInitalValue(item.defaultValue, selectOptions)
            : item.defaultValue;
        ...
        const getAttributeElement = (item: any): JSX.Element => {
          ...
          if (item.type !== "SELECT" && item.fixedValue?.value) {
            ...
            const newFormValue =
              item.type === "NUMBER"
                ? parseInt(item.fixedValue.value)
                : item.fixedValue.value;
            // 保证固定值被写入
          ...
          if (item.type === "NUMBER") {
            ...
            return (
              <CommandSelect
                label={
                  <FormLabel>
                    {getI18nConfigCurrentText(item.label, languageId)}
                  </FormLabel>
                }
                appId={appId}
                clusterId={clusterId}
                  ...
                  <div style={{ marginRight: "4px" }}>
                    <AdvancedFileSelectModal
                      ...
                      onSubmit={(path: string) => {
                        form.setFields([
                          { name: item.name, value: path, touched: true },
                        ]);
                        form.validateFields([item.name]);
                      }}
                      clusterId={clusterId}
                      scowdEnabled={
                        fullClusterConfigs[clusterId]?.scowd?.enabled
                      }
                    />
                  </div>
                }
        ...
        if (
          item.type === "SELECT" &&
          item.select.find((i) => i.requireGpu !== undefined)
        ) {
          const preValue = form.getFieldValue(item.name);

          if (preValue) {
            // 切换分区后看之前的版本是否还存在，若不存在，则选择版本的select的值置空
            const optionsContained = selectOptions.find(
              (i) => i.value === preValue
            );
  ...
  return (
    customFormItems.length > 0 && (
      <TitledSectionCard
        title={<SectionTitle>{t(p("appConfigSectionTitle"))}</SectionTitle>}
      >
