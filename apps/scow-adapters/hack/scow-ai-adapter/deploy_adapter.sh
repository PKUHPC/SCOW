...
function k8sDeploy() {
  log "========== 开始更新 adapter deploy image =========="
  # 1. 更新镜像
  log "更新 adapter 镜像..."
  cd /adapter/ &&  helm upgrade  scow-ai-adapter ./scow-ai-adapter-chart/ --set image.tag="$1" --set rolloutTrigger=$(date +%s) 2>&1 | tee -a "$LOG_FILE"
}
