...
const devHostEntryScript = `#!/bin/bash
# -------------------------------------------------------
...
func GetDevHostScriptHostDir(workingDirectory string) string {
	return filepath.Join(workingDirectory, DevHostScriptDirName)
}

func GetDevHostEntryScriptPath(workingDirectory string) string {
	return filepath.Join(GetDevHostScriptHostDir(workingDirectory), "entry.sh")
}

func EnsureDevHostEntryScript(workingDirectory string) error {
	if err := os.MkdirAll(GetDevHostScriptHostDir(workingDirectory), 0755); err != nil {
		return err
	}

	script, err := loadDevHostEntryScript()
	if err != nil {
		return err
	}

	return os.WriteFile(GetDevHostEntryScriptPath(workingDirectory), script, 0755)
}

func loadDevHostEntryScript() ([]byte, error) {
	script, err := os.ReadFile(DevHostEntryPath)
	if err == nil {
		logrus.Infof("Use dev host entry script from %s", DevHostEntryPath)
		return script, nil
	}
	if errors.Is(err, os.ErrNotExist) {
		logrus.Infof("Dev host entry script %s not found, use built-in script", DevHostEntryPath)
		return []byte(devHostEntryScript), nil
	}
	return nil, err
}
