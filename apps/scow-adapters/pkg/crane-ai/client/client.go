...
func InitClient() {
	...
	certDir, err := currentUserCertDir(CConfig)
	if err != nil {
		log.Fatalf("Failed to resolve Crane TLS certificate directory: %v", err)
	}
	uid := uint32(os.Getuid())
	CraneCtld, craneCtldConn, err = newCraneCtldClient(CConfig, certDir, &uid)
	if err != nil {
		log.Fatalf("Cannot create CraneCtld client: %v", err)
	}
	if CConfig.TLS.Enabled {
		userClients.startCleaner(defaultUserClientCleanupInterval)
	}
