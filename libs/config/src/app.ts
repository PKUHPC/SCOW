...
export const SlurmConfigSchema = Type.Object({
  options: Type.Array(Type.String({ description: "sbatch选项" }), { description: "运行slurm脚本时可添加的选项" }),
});
...
export const AppConfigSchema = Type.Object({
  ...
  reservedAppAttributes: Type.Optional(
    Type.Array(
      Type.Object({
        name: Type.Enum(ReservedAppAttributeName, {
          description:
            "系统保留APP表单字段名, 包括作业名，账户，集群，分区，Qos, 节点数，CPU卡数，GPU卡数，最长运行时间(分钟)",
        }),
    ...
    if (config.attributes) {
      config.attributes.forEach((item) => {
        ...
        if (item.type === AttributeType.select && item.fixedValue) {
          logger?.warn(
            `App ${id}'s form attributes of name ${item.name} is of type select, ` +
      ...
      config.reservedAppAttributes = Array.from(
        config.reservedAppAttributes
          .reduce((acc, item) => {
            ...
            // 系统保留APP表单字段值不能为空字符串
            if (
              (config.type === "fixedValue" && config.value === "") ||
            ...
            return acc;
          }, new Map<string, ReservedAppAttributeItem>())
          .values(),
      );
