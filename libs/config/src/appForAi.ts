...
export const AppConnectPropsSchema = Type.Object(
  {
    method: Type.Enum(
      { GET: "GET", POST: "POST" },
      { description: "连接所使用的HTTP方法" }
    ),
    path: Type.String({ description: "启动的相对路径" }),
    query: Type.Optional(
      Type.Record(Type.String(), Type.String(), { description: "query参数" })
    ),
    formData: Type.Optional(
      Type.Record(Type.String(), Type.String(), {
        description: "设置为POST时，需要以form data形式提交的数据",
      })
    ),
  },
  { description: "如何连接应用" }
...
export const WebAppConfigSchema = Type.Object({
  ...
  beforeScript: Type.String({
    description: "启动应用之前的准备命令。具体参考文档",
  }),
  ...
  script: Type.String({
    description: "启动应用的命令。可以使用beforeScript中定义的变量",
  }),
...
export const VncAppConfigSchema = Type.Object({
  beforeScript: Type.Optional(
    Type.String({ description: "启动应用之前的准备命令。具体参考文档" })
  ),
...
export const AppConfigSchema = Type.Object({
  ...
  logoPath: Type.Optional(
    Type.String({ description: "App应用图标的图片源路径" })
  ),
  image: Type.Optional(
    Type.Object({
    ...
    Type.Array(
      Type.Object({
        ...
        placeholder: Type.Optional(
          createI18nStringSchema({ description: "输入提示信息" })
        ),
        select: Type.Optional(
          Type.Array(
            Type.Object({
              value: Type.String({ description: "表单选项key，编程中使用" }),
              label: createI18nStringSchema({
                description: "表单选项展示给用户的文本",
              }),
              requireGpu: Type.Optional(
                Type.Boolean({ description: "表单选项是否只在分区为gpu时展示" })
              ),
            }),
            { description: "表单选项" }
          )
  ...
  appComment: Type.Optional(
    createI18nStringSchema({ description: "应用说明文字" })
  ),
});
...
export const getAiAppConfigs: GetConfigFn<Record<string, AppConfigSchema>> = (
  baseConfigPath,
  logger
) => {
  ...
  Object.entries(aiAppsConfig).forEach(([id, config]) => {
    if (!config[config.type]) {
      throw new Error(
        `App ${id} is of type ${config.type} but config.${config.type} is not set`
      );
    }
  });
